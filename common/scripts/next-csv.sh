...
gsed -i "s/$CURRENT_DEV_CSV/$NEW_DEV_CSV/g" $CSV_PATH/ibm-common-service-webhook.clusterserviceversion.yaml
TIME_STAMP="$(date '+%Y-%m-%dT%H:%M:%S'Z)"
gsed -i "s/2[0-9]*-[0-9]*-[0-9]*T[0-9]*:[0-9]*:[0-9]*Z/$TIME_STAMP/g" $CSV_PATH/ibm-common-service-webhook.clusterserviceversion.yaml
echo "Updated New file with new CSV version"
gsed -i "s/$PREVIOUS_DEV_CSV/$CURRENT_DEV_CSV/g" $CSV_PATH/ibm-common-service-webhook.clusterserviceversion.yaml
